 ...
 */
public class VisitorListRequest extends ResultPostExecute<List<VisitorModel>> {
    public void request(final int pageNo, final int pageSize){
        ...
        Call<ResponseBody> call = AppManager.getFollowService().getVisitorList(AppManager.getClientUser().sessionId, params);
    ...
    private void parseJson(String json){
        try {
            ...
            Type listType = new TypeToken<ArrayList<VisitorModel>>() {
            }.getType();
            Gson gson = new Gson();
            ArrayList<VisitorModel> visitorModels = gson.fromJson(result, listType);
            onPostExecute(visitorModels);
        } catch (Exception e) {
//            onErrorExecute(CSApplication.getInstance().getResources()
